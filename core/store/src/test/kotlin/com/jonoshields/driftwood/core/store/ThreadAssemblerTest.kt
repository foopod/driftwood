...
/** Thread assembly — fragmentation is the normal case, not an error: render what you have, not pretend gaps are failures. */
class ThreadAssemblerTest {
    ...
    fun `a reply with no parent attaches to the root and is not a gap`() {
        // A missing parent means the author targeted nothing specific, not a missing message.
        val r = root("hello")
    ...
    fun `a reply whose parent is not held flattens to the root and is marked`() {
        // The quiet "replying to a message not carried here" marker.
        val r = root("hello")
    ...
    fun `a thread whose root is not held still assembles`() {
        // Root ids outlive root content — a thread nobody holds the root of is normal, not damage.
