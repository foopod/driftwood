...
class MessageTest {
    ...
    fun `a reply may target a thread whose root is not held and name no parent`() {
        // A reply needs only the root id, not the root content itself.
    ...
    fun `the id is the hash of the preimage and excludes id and sig`() {
        val message = root()
        assertArrayEquals(
            com.jonoshields.driftwood.core.crypto.sha256(CanonicalCodec.encode(message.body)),
    ...
    fun `a message signed by one identity but claiming another is rejected`() {
        ...
        val forgedWire = MessageCodec.encode(
            Message.unverified(
                id = MessageId.of(com.jonoshields.driftwood.core.crypto.sha256(CanonicalCodec.encode(forgedBody))),
