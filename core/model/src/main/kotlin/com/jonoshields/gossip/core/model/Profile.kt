...
/** plan.md §3.5: profile format version. */
const val PROFILE_FORMAT_VERSION: Int = 1

/**
 * A signed claim: *this key calls itself this* (plan.md §3.5).
 *
 * Deliberately not a message. Names change and messages do not, and one row per identity
 * beats the same name repeated across every message that identity ever writes.
 *
 * Unlike [Message] there is no content hash: a profile is keyed by [author] and is mutable
 * by design, with the latest claim winning. It is signed by the key it names, so a relaying
 * peer cannot rewrite it — the worst a peer can do is withhold it.
 */
...
/**
 * The canonical form for profiles, following §3.2's encoding exactly — same length-prefixed
