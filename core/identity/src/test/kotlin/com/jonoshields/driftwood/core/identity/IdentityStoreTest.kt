...
/** Identity lifecycle against fakes for a fast JVM test; the real Keystore binding is covered separately by an instrumented test. */
class IdentityStoreTest {
    ...
    fun `the seed is never written in the clear`() {
        ...
        val seed = Mnemonic.decode(store.recoveryPhrase()).let {
            (it as com.jonoshields.driftwood.core.crypto.MnemonicResult.Success).seed
        }
    ...
    fun `phrase and public key stay consistent`() {
        val store = store()
        val author = store.create()
        val seed = (Mnemonic.decode(store.recoveryPhrase()) as com.jonoshields.driftwood.core.crypto.MnemonicResult.Success).seed
