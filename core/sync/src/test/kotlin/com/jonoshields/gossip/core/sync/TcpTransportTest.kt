...
/**
 * The protocol over an actual byte stream rather than the [Pipe] mock's in-memory channels —
 * real localhost sockets, a real kernel free to fragment or coalesce writes however it likes.
 * This is one layer more real than every other test in this suite, and it needs no device
 * (M3a, plan.md §7): TCP is TCP whether the two ends are on the same machine or two phones on
