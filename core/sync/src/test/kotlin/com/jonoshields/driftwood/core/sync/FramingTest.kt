...
/** The framing layer is the first thing a hostile peer touches, so these lean hard on the rejection side. */
class FramingTest {
    ...
    fun `every record type round trips`() {
        ...
        assertEquals(Record.BatchDone, roundTrip(Record.BatchDone))
    ...
    fun `records that carry nothing must carry nothing`() {
        ...
        assertMalformed(byteArrayOf(0x0B) + intBytes(1) + byteArrayOf(0), "BATCH_DONE with a payload")
    }
