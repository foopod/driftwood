...
internal class Recording(private val inner: Connection) : Connection {
    ...
    fun messageIdsSent(): List<MessageId> = sent
        ...
        .mapNotNull { (com.jonoshields.driftwood.core.model.MessageVerifier.verify(it.wire)
            as? com.jonoshields.driftwood.core.model.VerifyResult.Valid)?.message?.id }
}
...
internal suspend fun sync(
    ...
    batchSize: Int = PHASE_BATCH_SIZE,
    ...
    val initiator = async {
        Session(initiatorStore, clock, batchSize = batchSize).run(Role.INITIATOR, a, initiatorDevice, confirm)
    }
    val responder = async {
        Session(responderStore, clock, batchSize = batchSize).run(Role.RESPONDER, b, responderDevice, confirm)
    }
...
/** Runs our session against a peer we script frame by frame, for cases where the peer misbehaves in a way a real one never would. */
internal suspend fun againstScriptedPeer(
    ...
    val ours = async {
        Session(store, Clock.fixed(now), batchSize = batchSize).run(Role.RESPONDER, ourEnd, responderDevice, confirm)
    }
    val result = ours.await()
    // The session doesn't close the connection itself — the caller owns it — so both ends are closed here.
    ourEnd.close()
    peerEnd.close()
    peer.await()
    result
...
/** A peer that follows the protocol exactly, holds nothing, and delivers whatever it's told — used to feed content a real store wouldn't. */
