...
class PipeTest {
    ...
    fun `both sides writing before either reads will deadlock, as on a real socket`() {
        // This is the failure the session's turn-taking exists to prevent — an unbounded pipe would hide it here.
        val (a, b) = Pipe.open(capacity = 1)
...
/** Real blocking I/O on real dispatchers, so these use `runBlocking` + a wall-clock JUnit timeout rather than `runTest`'s virtual clock. */
